...
//		debug("Found file '%s'. Namelen=%x fileLenComp=%x, compr=%d flags=%d\n", 
...
//			debug("Alloc %p\n", r);
			...
			if (h.compression==COMPRESS_NONE) {
				...
				r->posComp++;
				debug("Heatshrink compressed file; decode parms = %x\n", parm);
...
//		debug("Reading %d bytes from %x\n", len, (unsigned int)fh->posComp);
		...
		fh->posDecomp+=len;
		fh->posComp+=len;
//		debug("Done reading %d bytes, pos=%x\n", len, fh->posComp);
		return len;
#ifdef ESPFS_HEATSHRINK
